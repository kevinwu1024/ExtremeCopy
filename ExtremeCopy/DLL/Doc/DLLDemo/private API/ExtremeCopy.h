...
typedef int (__stdcall *lpfnCopyRoutineA_t)(int nCmd,int nParam1,int nParam2,const char* pSrcFile,const char* pDstFile) ;
typedef int (__stdcall *lpfnCopyRoutineW_t)(int nCmd,int nParam1,int nParam2,const WCHAR* pSrcFile,const WCHAR* pDstFile) ;
...
// error process
#define ErrorHandlingFlag_Retry			(1 << 1)		// retry 
#define ErrorHandlingFlag_Ignore		(1 << 2)		// ignore
#define ErrorHandlingFlag_Exit			(1 << 3)		// exit

// same file name process
#define SameFileProcess_Replace			1	// replace
#define SameFileProcess_Skip			0	// skip
#define SameFileProcess_Rename			3	// rename
...
//void __stdcall ExtremeCopy_SetNotEvaluateA(const char* lpstrSN) ;
	...
	//			If invoke successful it will reutrn true, otherwise false
	//
	bool __stdcall ExtremeCopy_StartA(int RunType,bool bSyncOrAsyn,lpfnCopyRoutineA_t lpfnCopyRoutineA=NULL) ;
	bool __stdcall ExtremeCopy_StartW(int RunType,bool bSyncOrAsyn,lpfnCopyRoutineW_t lpfnCopyRoutineW=NULL) ;
