...
namespace demo
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            ExtremeCopy.ExtremeCopy_StartW(ExtremeCopy.XCRunType_Copy, true, ExtremeCopyRoutine, IntPtr.Zero); // start to run copy work
        }

        static int ExtremeCopyRoutine(int nCmd,IntPtr pRoutineParam, int nParam1, int nParam2, string strSrcFile, string strDstFile)
        {
            int nRet = 0;
            ...
            switch (nCmd)
            {
                ...
                case ExtremeCopy.ROUTINE_CMD_SAMEFILENAME: // same file name
                    ...
                    nRet = ExtremeCopy.SameFileProcess_Skip; // skip
                    break;
